...
** youtube music
*** pacman
#+begin_src sh :tangle youtube_music.sh
#!/bin/bash

# GitHub 仓库信息
REPO="th-ch/youtube-music"
PKG_NAME="youtube-music"
...
# 检查是否已安装旧版本
INSTALLED_VERSION=$(pacman -Qi "$PKG_NAME" 2>/dev/null | grep 'Version' | awk '{print $3}')
...
# 下载 .pacman 包
DOWNLOAD_URL="https://github.com/$REPO/releases/download/$LATEST_VERSION/$PKG_NAME-${LATEST_VERSION#v}.pacman"
TEMP_PKG="/tmp/$PKG_NAME-${LATEST_VERSION#v}.pacman"

echo "⬇️ 下载 $DOWNLOAD_URL ..."
if ! curl -L -o "$TEMP_PKG" "$DOWNLOAD_URL"; then
    echo "❌ 下载失败！"
    exit 1
fi
...
# 安装
echo "🔧 正在安装..."
sudo pacman -U --noconfirm "$TEMP_PKG"
